...
const GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        heading: '📥 Import from Paper Sheets',
        body: 'Bring backlog paper scouting sheets into Pestlook without re-typing them one observation at a time. Click <strong>📥 Import from Paper</strong> above the sessions table to open the import wizard.<br><br>' +
          '<strong>1. Download a template</strong> — choose a Farm and Field, then click <strong>⬇ Download Template</strong>. You get an <code>.xlsx</code> file pre-filled with that Field\'s name, its enabled traps, and dropdown lists of your pest catalogue and team members. The Farm/Field cells are locked so they can\'t be changed by accident. One sheet always represents <strong>one session</strong> — the Session Date and Scout are entered once at the top, not repeated per row.<br><br>' +
          '<strong>2. Fill it in offline</strong> — enter the Session Date (format <code>YYYY-MM-DD</code>, with an example shown next to the cell and a tooltip on selection), the Scout\'s name, weather, temperature, and notes at the top, then one row per observation below. Trap Name, Pest, Life Stage, Capture Mode, and Yes/No fields are all dropdown lists sourced from your live data — free typing is only needed for counts, GPS, and notes. If a date or time is typed in the wrong format, Excel shows a custom error message explaining the expected format before it will accept the cell.<br><br>' +
          '<strong>3. Upload and check</strong> — back in Pestlook, choose the completed file and click <strong>Check Sheet</strong>. Every row is validated against your real traps, pests, and team members before anything is saved; problem rows are listed with the exact reason (e.g. an unrecognised trap name).<br><br>' +
          '<strong>4. Confirm Import</strong> — once the sheet is clean, click <strong>✓ Confirm Import</strong> to create the session and all its observations in one go.<br><br>' +
          '<strong>Temperature units</strong> — the template\'s temperature column automatically labels itself °C or °F to match the temperature unit set in your account preferences, and the value is converted to Celsius for storage on import.<br><br>' +
          '<strong>Timezone</strong> — the Session Date and each observation\'s time are wall-clock times in the <strong>scout\'s</strong> timezone (the team member selected in the Scout field, not necessarily the person uploading the file). Pestlook converts them to UTC using that scout\'s saved timezone before saving, so sessions land on the correct calendar day and time regardless of where the file is uploaded from.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        intro: 'If the same pest keeps breaching its threshold on the same field year after year, something is not working. This tab flags those combinations. It does not record treatment history — it simply looks at whether counts exceeded the configured threshold in each calendar year and asks whether the problem is getting better, worse, or staying the same. A worsening trend across multiple years is the clearest signal that your current control method has lost effectiveness and that a rotation or resistance test is overdue.',
        items: [
          {
            heading: 'KPI Cards',
            body: '<strong>Patterns Found</strong> — total field × pest combinations with breaches recorded across at least 2 distinct calendar years in the selected date range. <strong>Likely Resistance</strong> — 3 or more breach years with a non-improving trend, or 2 breach years with a worsening trend. <strong>Possible Resistance</strong> — exactly 2 breach years. The years-analysed figure shows how many calendar years the date range covers.',
          },
          {
            heading: 'Resistance Risk badge — Likely / Possible',
            body: '<span style="color:#c0392b;font-weight:600;">Likely</span> — treat this as a probable resistance situation. Rotate mode-of-action before the next treatment window. Do not apply the same product group again until resistance testing or a full-season break has been completed. Discuss with your agronomist. <span style="color:#e67e22;font-weight:600;">Possible</span> — two breach years recorded. Could be a bad season, seasonal population variation, or early resistance development. Evaluate whether the same control method was used both years and consider alternating.',
          },
          {
            heading: 'Trend badge — Worsening / Stable / Improving',
            body: 'The trend compares the breach rate in the <strong>last</strong> year of the pattern to the breach rate in the <strong>first</strong> year — it does not weight intermediate years. A spike in the middle followed by a drop still shows as Improving if the final year is lower than the first. Use the per-season bar chart to see the full picture. <span style="color:#c0392b;font-weight:600;">📈 Worsening</span> — final year breach rate is ≥ 15 percentage points higher than the first year. <span style="color:#27ae60;font-weight:600;">📉 Improving</span> — down ≥ 15 pp. <span style="color:#e67e22;font-weight:600;">➡️ Stable</span> — within 15 pp either way.',
          },
          {
            heading: 'Breach Rate by Season bars',
            body: 'Each year shows the breach rate (percentage of observation records where the count exceeded the threshold), the number of breaches, the total observation records, and the average count. Note: the denominator is observation records, not distinct scouting visits — if one session produces multiple observation records for the same pest, they all count. This is consistent year-over-year so the trend comparison is valid, but the absolute percentage does not equal "X% of your scouting visits had a breach." Bars are green below 30%, amber 30–59%, red 60% and above.',
          },
          {
            heading: 'Recommendation panel',
            body: 'Likely patterns recommend rotating mode-of-action, resistance testing if available, and considering biocontrol or cultural controls. Possible patterns suggest evaluating the current treatment and alternating methods. These are starting points — your agronomist should confirm the rotation strategy based on the specific products used.',
          },
          {
            heading: 'No treatment records required',
            body: 'This tab works without any treatment history in Pestlook. It is based purely on observation counts vs configured thresholds. A pattern will be flagged whether or not any treatment was applied — a field with recurring breaches and no treatment logged is itself a red flag worth investigating.',
          },
          {
            heading: 'How the date filter works',
            body: 'The date filter is applied strictly — only observations within the selected range are included. This means <strong>the range must span at least two calendar years</strong> for any patterns to appear; a range entirely within one calendar year will return nothing because no combination can have breaches in two distinct years. The default range is two years (01 Jul of two years ago to today) for this reason. If you narrow the filter to a single season, the tab will appear empty even if you have known multi-year breach data — widen the range back to at least 24 months.',
          },
          {
            heading: 'Nothing shown — or fewer patterns than expected',
            body: 'Check three things: (1) the date range spans at least two calendar years — use the date pickers to extend it if needed; (2) action thresholds are configured on your pests (set via the Pests page) — without a threshold no breach can be detected; (3) sessions are linked to a specific field rather than just a farm. If all three are met but patterns are still missing, the pest may have only breached in a single year within the selected window.',
          },
        ],
      },
    ],
...
const FAQ = {
  ...
  containment: [
    ...
    { q: 'Resistance Patterns shows no patterns even though I know we have had repeated breaches.', a: 'The date filter is applied strictly, and patterns require breaches in at least 2 distinct calendar years within that range. A filter set to a single season will always return empty. Make sure: (1) the date range spans at least 24 months — extend it with the date pickers; (2) action thresholds are configured on your pests (set via the Pests page); (3) sessions are linked to specific fields rather than just a farm.' },
